...
const claude263SHA = "ef5d2909c8af49f31ab6d5487e90316777bc2fac170adfe8160716caa8aaf4f9"

func TestClaude263PatchRequiresExactVersionOSArchAndSHA(t *testing.T) {
	patch := findClaudeUIPatch("2.1.263", claude263SHA)
	if runtime.GOOS == "darwin" && runtime.GOARCH == "arm64" {
		if patch == nil {
			t.Fatal("expected verified Claude 2.1.263 darwin/arm64 patch to match")
		}
	} else if patch != nil {
		t.Fatalf("patch matched unsupported runtime %s/%s", runtime.GOOS, runtime.GOARCH)
	}
	if got := findClaudeUIPatch("2.1.263", claude261SHA); got != nil {
		t.Fatalf("Claude 2.1.263 patch matched wrong SHA: %#v", got)
	}
	if got := findClaudeUIPatch("2.1.261", claude263SHA); got != nil {
		t.Fatalf("Claude 2.1.263 SHA matched wrong version: %#v", got)
	}
}

func TestClaude263WrongSHAFallsBackToUnpatchedExecutable(t *testing.T) {
	claudePath := t.TempDir() + "/2.1.263"
...
func TestClaude263ModelPickerContainsExactlyThreeCodexTiers(t *testing.T) {
	data := []byte(`function fdn(e=!1){` + strings.Repeat(" ", 5000) + `function lto(e,t){}`)
	if !patchModelPickerOptions_2_1_263(data, modelconfig.Default()) {
		t.Fatal("patchModelPickerOptions_2_1_263 reported no changes")
	}
	got := string(data)
	...
	if !strings.Contains(got, `function Rue(e=!1,t=null){return CDXOpts263(e,t)}`) {
...
func TestClaude263ModelPickerRejectsOverrideAndCustomRows(t *testing.T) {
	data := []byte(claude263ModelOptionsOverrideTarget + claude263ModelSelectionSourceTarget + claude263ModelExtraOptionsTarget + claude263ModelPickerValueTarget)
	if !patchModelPickerExtraOptions_2_1_263(data) {
		t.Fatal("model picker extra-options patch did not apply")
	}
	got := string(data)
	for _, want := range []string{`Ct=V(()=>fdn(Zt),[Zt])`, `zt=to===null?Qw:umt(Ct,to)??to,`, `Go=Ct.slice(0,3)`, `options:Fn.slice(0,3)`} {
		if !strings.Contains(got, want) {
			t.Fatalf("model picker missing %q", want)
		}
	}
	...
	for _, forbidden := range []string{"Current model", "Base model", "Oe??fdn"} {
		if strings.Contains(got, forbidden) {
...
func TestClaude263ModelPickerResolverReturnsOnlyThreeCanonicalRows(t *testing.T) {
	data := []byte(`function fto(e,t){` + strings.Repeat(" ", 5000) + `function mdn(e){}`)
	if !patchModelPickerResolver_2_1_263(data) {
		t.Fatal("model picker resolver patch did not apply")
	}
	if !strings.Contains(string(data), `function fto(e,t){return CDXOpts263(e,t).slice(0,3)}`) {
		t.Fatal("model picker resolver retained noncanonical option sources")
	}
}

func TestClaude263LogoPatchFailsClosedOnOverflow(t *testing.T) {
	data := []byte(`function PHe(){let l=a.DEMO_VERSION??` + strings.Repeat(" ", 1000) + `function VQt(l,b,x){}`)
	original := append([]byte(nil), data...)
	if patchLogoDisplayDataFunction_2_1_263(data, strings.Repeat("x", 4000), "2.1.263") {
...
func TestClaude263PatchTargetsMaintenanceBinary(t *testing.T) {
	if version := os.Getenv("CLAUDODEX_MAINTENANCE_CLAUDE_VERSION"); version != "" && version != "2.1.263" {
		return
	}
	...
	if got := sha256Hex(source); got != claude263SHA {
		t.Fatalf("maintenance Claude SHA = %s, want %s", got, claude263SHA)
	}
	if !validateClaude209UIBrandingReplacements(source, claude263UIBrandingReplacements) {
		...
		t.Fatal("Claude 2.1.263 branding prerequisites do not match the maintenance binary")
	}
	records, hashes, ok := claude263EmbeddedBunModuleHashes(source)
	if !ok {
		t.Fatal("Claude 2.1.263 Bun module table is unavailable")
	}
	patched := append([]byte(nil), source...)
	if !applyClaudeUIPatches_2_1_263(patched, "test", "2.1.263", modelconfig.Default()) {
		for _, transformation := range claude263SourceTransformationsForConfig("test", "2.1.263", modelconfig.Default()) {
			...
			if !transformation.apply(candidate) {
				...
				if transformation.name == "remote-control" {
					for _, remote := range claude263RemoteControlTransformations() {
		...
		t.Fatal("complete Claude 2.1.263 patch did not apply")
	}
	for _, transformation := range claude263Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			...
			if !transformation.apply(data) {
				t.Errorf("Claude 2.1.263 target %s does not match the maintenance binary", transformation.name)
			}
		})
	}
	for _, transformation := range claude263RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("Claude 2.1.263 remote-control target %s does not match the maintenance binary", transformation.name)
			}
		})
	}
	if tiers := claude263ModelPickerTierCount(patched); tiers != 3 {
	...
	for _, want := range []string{"Claudodex Info", "test using Claude Code v2.1.263", "function RH(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}"} {
		if !bytes.Contains(patched, []byte(want)) {
			t.Fatalf("complete patch missing %q", want)
		}
	}
	start := bytes.Index(patched, []byte("function CDX263("))
	if start < 0 {
		t.Fatal("patched model picker start is missing")
	}
	endRelative := bytes.Index(patched[start:], []byte("function lto("))
	if endRelative < 0 {
