...
func TestInstalledClaudeLiveCodexModelEfforts(t *testing.T) {
	...
	for _, slug := range modelconfig.Default().RequiredModels() {
		model, ok := catalogModel(models, slug)
		if !ok {
			t.Fatalf("live Codex model metadata missing %s", slug)
		}
		advertised := make(map[string]bool, len(model.SupportedReasoningLevels))
		for _, level := range model.SupportedReasoningLevels {
			advertised[level.Effort] = true
		}
		for _, effort := range wantEfforts {
			if !advertised[effort] {
				t.Fatalf("live Codex model %s does not advertise Claude effort %s", slug, effort)
			}
			mapped := convert.MapReasoningEffort(slug, effort, 0)
			if !advertised[string(mapped)] {
				t.Fatalf("Claude effort %s maps to unadvertised %s level %s", effort, slug, mapped)
			}
		}
		if mapped := convert.MapReasoningEffort(slug, "ultracode", 0); mapped != convert.EffortMax || !advertised[string(mapped)] {
			t.Fatalf("Claude ultracode maps to %s for %s, want advertised max", mapped, slug)
		}
	}
}
...
func TestInstalledClaudeUIPatchSmoke(t *testing.T) {
	...
	case "2.1.282":
		wants = append(wants, "function RN(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.281":
		wants = append(wants, "function sN(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.278":
	...
	case "2.1.274":
		wants = append(wants, "function XO(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.273":
		wants = append(wants, "function aO(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.263":
		wants = append(wants, "function RH(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.261":
		wants = append(wants, "function RH(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.260":
		wants = append(wants, "function TH(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.259":
		wants = append(wants, "function p0(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.258":
		wants = append(wants, "function QH(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.252":
		wants = append(wants, "function _H(){return process.env.CLAUDE_BRIDGE_OAUTH_TOKEN}")
	case "2.1.251":
	...
	if claudeVersion == "2.1.216" || claudeVersion == "2.1.218" || claudeVersion == "2.1.219" || claudeVersion == "2.1.220" || claudeVersion == "2.1.221" || claudeVersion == "2.1.222" || claudeVersion == "2.1.223" || claudeVersion == "2.1.226" || claudeVersion == "2.1.227" || claudeVersion == "2.1.228" || claudeVersion == "2.1.229" || claudeVersion == "2.1.233" || claudeVersion == "2.1.234" || claudeVersion == "2.1.245" || claudeVersion == "2.1.246" || claudeVersion == "2.1.247" || claudeVersion == "2.1.251" || claudeVersion == "2.1.252" || claudeVersion == "2.1.258" || claudeVersion == "2.1.259" || claudeVersion == "2.1.260" || claudeVersion == "2.1.261" || claudeVersion == "2.1.263" || claudeVersion == "2.1.273" || claudeVersion == "2.1.274" || claudeVersion == "2.1.276" || claudeVersion == "2.1.278" || claudeVersion == "2.1.281" || claudeVersion == "2.1.282" || claudeVersion == "2.1.283" {
		...
		case "2.1.283":
			...
			tierCall = `r("`
		case "2.1.282":
			normalizer = "function CDX282("
			pickerEnd = "function zr("
			tierCall = `r("`
		case "2.1.281":
			normalizer = "function CDX281("
			pickerEnd = "function or("
			tierCall = `r("`
		case "2.1.278":
			normalizer = "function CDX278("
			pickerEnd = "function lS("
			tierCall = `r("`
		case "2.1.276":
			normalizer = "function CDX276("
			pickerEnd = "function BC("
			tierCall = `r("`
		case "2.1.274":
			normalizer = "function CDX274("
			pickerEnd = "function Kx("
			tierCall = `r("`
		case "2.1.273":
			normalizer = "function CDX273("
			pickerEnd = "function yR("
			tierCall = `r("`
		case "2.1.263":
			normalizer = "function CDX263("
			pickerEnd = "function lto("
			tierCall = `r("`
		case "2.1.261":
			normalizer = "function CDX261("
			pickerEnd = "function rto("
			tierCall = `r("`
		case "2.1.260":
			normalizer = "function CDX260("
			pickerEnd = "function dQr("
			tierCall = `r("`
		case "2.1.259":
			normalizer = "function CDX259("
			pickerEnd = "function _ro("
			tierCall = `r("`
		case "2.1.258":
			normalizer = "function CDX258("
			pickerEnd = "function X9r("
			tierCall = `r("`
		case "2.1.252":
			normalizer = "function CDX252("
		...
		if tiers := bytes.Count(picker, []byte(tierCall)); tiers != 3 {
			t.Fatalf("patched installed Claude picker tier count = %d, want 3", tiers)
		}
		for _, want := range []string{tierCall + `opus",`, tierCall + `sonnet",`, tierCall + `haiku",`, "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"} {
	...
	case "2.1.282":
		brandingReplacements = claude282UIBrandingReplacements
	case "2.1.281":
		brandingReplacements = claude281UIBrandingReplacements
	case "2.1.278":
		brandingReplacements = claude278UIBrandingReplacements
	case "2.1.276":
		brandingReplacements = claude276UIBrandingReplacements
	case "2.1.274":
		brandingReplacements = claude274UIBrandingReplacements
	case "2.1.273":
		brandingReplacements = claude273UIBrandingReplacements
	case "2.1.263":
		brandingReplacements = claude263UIBrandingReplacements
	case "2.1.261":
		brandingReplacements = claude261UIBrandingReplacements
	case "2.1.260":
		brandingReplacements = claude260UIBrandingReplacements
	case "2.1.259":
		brandingReplacements = claude259UIBrandingReplacements
	case "2.1.258":
		brandingReplacements = claude258UIBrandingReplacements
	case "2.1.252":
		brandingReplacements = claude252UIBrandingReplacements
	case "2.1.251":
...
func TestArchivedClaude251PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.251" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.251") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
...
func TestArchivedClaude252PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.252" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.252") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want a registered supported version", version)
	...
	for _, transformation := range claude252Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.252", transformation.name)
			}
		})
	}
	for _, replacement := range claude252UIBrandingReplacements {
		if got := bytes.Count(source, []byte(replacement.old)); got != replacement.expectedCount {
			t.Errorf("branding count for %q = %d, want %d", replacement.old, got, replacement.expectedCount)
		}
	}
...
func TestArchivedClaude258PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.258" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.258") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.258", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	for _, transformation := range claude258Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.258", transformation.name)
			}
		})
	}
	for _, transformation := range claude258RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.258", transformation.name)
			}
		})
	}
	for _, target := range []struct {
		...
		{"Run claude --resume to pick a session, or start a new one.", 2},
	} {
		if got := bytes.Count(source, []byte(target.value)); got != target.want {
			t.Errorf("resume target count for %q = %d, want %d", target.value, got, target.want)
		}
	}
	for _, replacement := range claude258UIBrandingReplacements {
		if got := bytes.Count(source, []byte(replacement.old)); got != replacement.expectedCount {
			t.Errorf("branding count for %q = %d, want %d", replacement.old, got, replacement.expectedCount)
		}
	}
	t.Run("all-required-transformations-gate", func(t *testing.T) {
		broken := bytes.Replace(append([]byte(nil), source...), []byte("function fHe(){let l=a.DEMO_VERSION??"), []byte("function fHe(){let l=MISSING_TARGET??"), 1)
		if applyClaudeUIPatches_2_1_258(broken, "test", "2.1.258", modelconfig.Default()) {
...
func TestArchivedClaude259PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.259" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.259") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.259", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	if got := sha256Hex(source); got != claude259SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude259SHA)
	}
	for _, transformation := range claude259Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.259", transformation.name)
			}
		})
	}
	for _, transformation := range claude259RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.259", transformation.name)
			}
		})
	}
	for _, replacement := range claude259UIBrandingReplacements {
		if got := bytes.Count(source, []byte(replacement.old)); got != replacement.expectedCount {
			t.Errorf("branding count for %q = %d, want %d", replacement.old, got, replacement.expectedCount)
		}
	}
...
func TestArchivedClaude260PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.260" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.260") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.260", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	if got := sha256Hex(source); got != claude260SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude260SHA)
	}
	for _, transformation := range claude260Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.260", transformation.name)
			}
		})
	}
	for _, transformation := range claude260RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.260", transformation.name)
			}
		})
	}
...
func TestArchivedClaude261PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.261" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.261") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.261", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	if got := sha256Hex(source); got != claude261SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude261SHA)
	}
	for _, transformation := range claude261Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.261", transformation.name)
			}
		})
	}
	for _, transformation := range claude261RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.261", transformation.name)
			}
		})
	}
...
func TestArchivedClaude263PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.263" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.263") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.263", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	if got := sha256Hex(source); got != claude263SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude263SHA)
	}
	for _, transformation := range claude263Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.263", transformation.name)
			}
		})
	}
	for _, transformation := range claude263RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.263", transformation.name)
			}
		})
	}
	for _, replacement := range claude263UIBrandingReplacements {
		if got := bytes.Count(source, []byte(replacement.old)); got != replacement.expectedCount {
			t.Errorf("branding count for %q = %d, want %d", replacement.old, got, replacement.expectedCount)
		}
	}
	t.Run("all-required-transformations-gate", func(t *testing.T) {
		broken := bytes.Replace(append([]byte(nil), source...), []byte("function PHe(){let l=a.DEMO_VERSION??"), []byte("function PHe(){let l=MISSING_TARGET??"), 1)
		if applyClaudeUIPatches_2_1_263(broken, "test", "2.1.263", modelconfig.Default()) {
			t.Fatal("patch succeeded without the required logo transformation target")
		}
	})
}
...
func TestArchivedClaude273PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.273" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.273") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.273", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	...
	if got := sha256Hex(source); got != claude273SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude273SHA)
	}
	for _, transformation := range claude273Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.273", transformation.name)
			}
		})
	}
	for _, transformation := range claude273RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.273", transformation.name)
			}
		})
	}
...
func TestArchivedClaude274PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.274" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.274") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.274", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	...
	if got := sha256Hex(source); got != claude274SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude274SHA)
	}
	for _, transformation := range claude274Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.274", transformation.name)
			}
		})
	}
	for _, transformation := range claude274RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.274", transformation.name)
			}
		})
	}
...
func TestArchivedClaude276PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.276" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.276") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.276", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	}
	if got := sha256Hex(source); got != claude276SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude276SHA)
	}
	for _, transformation := range claude276Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.276", transformation.name)
			}
		})
	}
	for _, transformation := range claude276RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.276", transformation.name)
			}
		})
	}
	for _, replacement := range claude276UIBrandingReplacements {
		if got := bytes.Count(source, []byte(replacement.old)); got != replacement.expectedCount {
			t.Errorf("branding count for %q = %d, want %d", replacement.old, got, replacement.expectedCount)
		}
	}
...
func TestInstalledClaude278PatchTargets(t *testing.T) {
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.278" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.278") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
...
func TestInstalledClaude281PatchTargets(t *testing.T) {
	if os.Getenv("CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE") != "1" {
		t.Skip("set CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE=1 to run installed Claude smoke test")
	}
	claudePath, err := exec.LookPath("claude")
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.281" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.281") {
			t.Logf("installed Claude %s targets are covered by its version-specific test", version)
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.281", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	...
	if got := sha256Hex(source); got != claude281SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude281SHA)
	}
	for _, transformation := range claude281Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.281", transformation.name)
			}
		})
	}
	for _, transformation := range claude281RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.281", transformation.name)
			}
		})
	}
...
func TestInstalledClaude282PatchTargets(t *testing.T) {
	if os.Getenv("CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE") != "1" {
		t.Skip("set CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE=1 to run installed Claude smoke test")
	}
	claudePath, err := exec.LookPath("claude")
	...
	if version := detectClaudeVersion(context.Background(), claudePath); version != "2.1.282" {
		if installedClaudeTargetsCoveredByLaterTest(version, "2.1.282") {
			return
		}
		t.Fatalf("installed Claude version = %s, want 2.1.282", version)
	}
	source, err := os.ReadFile(claudePath)
	if err != nil {
		t.Fatal(err)
	...
	if got := sha256Hex(source); got != claude282SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude282SHA)
	}
	for _, transformation := range claude282Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.282", transformation.name)
			}
		})
	}
	for _, transformation := range claude282RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.282", transformation.name)
			}
		})
	}
...
func TestInstalledClaude283PatchTargets(t *testing.T) {
	if os.Getenv("CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE") != "1" {
		t.Skip("set CLAUDODEX_RUN_INSTALLED_CLAUDE_SMOKE=1 to run installed Claude smoke test")
	}
	claudePath, err := exec.LookPath("claude")
	...
	if got := sha256Hex(source); got != claude283SHA {
		t.Fatalf("installed Claude SHA = %s, want %s", got, claude283SHA)
	}
	for _, transformation := range claude283Transformations("test") {
		t.Run(transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Fatalf("%s patch target did not match installed Claude 2.1.283", transformation.name)
			}
		})
	}
	for _, transformation := range claude283RemoteControlTransformations() {
		t.Run("remote-control/"+transformation.name, func(t *testing.T) {
			data := append([]byte(nil), source...)
			if !transformation.apply(data) {
				t.Errorf("remote-control %s patch target did not match installed Claude 2.1.283", transformation.name)
			}
		})
	}
